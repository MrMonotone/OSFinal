...
Scheduler_p Scheduler_constructor(){
    ...
    n_schedule->priority_ready_queue = priority_queue_constructor();
...
void Scheduler_destructor(Scheduler_p scheduler){
    ...
    priority_queue_destructor(scheduler->priority_ready_queue);
...
void Scheduler_timer_handler(void *scheduler, CPU_p cpu){
    ...
    PCB_p temp_pcb = NULL;
    if(temp->running_queue->head){
        temp_pcb = (PCB_p)(temp->running_queue->head->data);
        PCB_set_p_state(temp_pcb, READY);
         Scheduler_transfer_node(temp->running_queue, temp->priority_ready_queue->queue[temp_pcb->priority]);  
    }
...
void Scheduler_io_request_2_handler(void *scheduler, CPU_p cpu){
    if(pthread_mutex_trylock(&timer_lock) != EBUSY ){
        Scheduler_p temp = (Scheduler_p)(scheduler);
        if(temp->running_queue->head != NULL){
        PCB_p temp_pcb = (PCB_p)(temp->running_queue->head->data);
        PCB_set_p_state(temp_pcb, BLOCKED);   
...
void Scheduler_io_completion_1_handler(void *scheduler, CPU_p cpu){
    ...
    if(pthread_mutex_trylock(&timer_lock) != EBUSY){
        ...
        Scheduler_p temp = (Scheduler_p)(scheduler);
        if(temp->io_1_waiting_queue->head){
            PCB_p temp_pcb = (PCB_p)(temp->io_1_waiting_queue->head->data);
            PCB_set_p_state(temp_pcb, READY);
            Scheduler_transfer_node(temp->io_1_waiting_queue, temp->priority_ready_queue->queue[temp_pcb->priority]);
        }
...
void Scheduler_io_completion_2_handler(void *scheduler, CPU_p cpu){
    if(pthread_mutex_trylock(&timer_lock) != EBUSY){
        Scheduler_p temp = (Scheduler_p)(scheduler);
        if(temp->io_2_waiting_queue->head){
            PCB_p temp_pcb = (PCB_p)(temp->io_2_waiting_queue->head->data);
            PCB_set_p_state(temp_pcb, READY);
            Scheduler_transfer_node(temp->io_2_waiting_queue, temp->priority_ready_queue->queue[temp_pcb->priority]);
            pthread_mutex_unlock(&timer_lock);
        }
...
void Scheduler_dispatcher(Scheduler_p scheduler, CPU_p cpu){
    //Node_p transfer = (Node_p)priority_queue_dequeue(scheduler->priority_ready_queue);
    FIFO_queue_p level = priority_queue_find_head(scheduler->priority_ready_queue);
    PCB_p process = NULL;
    if(level && level->head){
        process = level->head->data;
        PCB_set_p_state(process, RUNNING);
    }
     if(cpu->sys_stack->head){
             unsigned int *sw = (void*)&cpu->sys_stack->head->data;
             unsigned int *pc = (void*)&cpu->sys_stack->head->next->data;
    
             *sw = (unsigned int)process->sw;
            *pc = (unsigned int)process->pc;
     }
    Scheduler_transfer_node(level, scheduler->running_queue);
